...
2. **Select** — filter: common stock only (no ETFs/funds/preferreds/units), **exchange-listed only** (NYSE/NASDAQ/AMEX; OTC/pink-sheet lines excluded, 2026-07-02 — they're not the "US stocks at new highs" the audience means and chart-img can't chart them), market cap > $1B (enforced both in the source screen and independently in `select.pick`), not blocked by the cooldown rule. Rank survivors by market cap descending.
...
5. **Publish** — compose text (`$TICKER printed a new 52-week high today` — deliberately no price/%chg/market cap, since those numbers go stale between post time and read time; the attached chart carries the quantitative story) and hand to the `Publisher`. Phase 1: dry-run writer to `output/YYYY-MM-DD/`. Phase 2: Stocktwits API client. (Copy decision 2026-07-02.)
